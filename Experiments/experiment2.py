...
def replace_nexts_of_positives(x_train,vectors_to_modify,replacement):
    for i in range(0,len(x_train)):
        curr_vec = x_train[i]
        if (vectors_to_modify == curr_vec).all(1).any():
            x_train[i] = replacement
    return x_train
...
def test_model_cyber(vectors,all_targets, vectors_to_modify, replacement):
    x_train,y_train,x_test,y_test = get_balanced_train(vectors,all_targets)
    
    # edit x_train so that all sentences that came after a positive target
    # will be set to mean of those sentences ...
    x_train = replace_nexts_of_positives(x_train,vectors_to_modify,replacement)
...
def main():
    ...
    positive_target_nexts = get_positive_target_nexts(sents_ll,all_sents,sent_vectors,all_targets)
    new_mean = map(mean, zip(*positive_target_nexts))
    
    test_model_cyber(sent_vectors,all_targets,positive_target_nexts, new_mean)


# identify all_vectors that are right after positive targets
# calculate their average
# match those vectors with vectors in x_train, and replace with average
main()
