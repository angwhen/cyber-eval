...
#returns vectors of next sents with their associated curr targets
def get_next_sents(sents_ll,all_sents,sent_vectors,all_targets):
    curr_node = sents_ll.head
    next_sents = []
    curr_targets = []
    index_dif = 0 #account for eof index difference, not in sents ll
    while (curr_node!=None and curr_node.reference != None):
        ...
        if not is_eof(curr_node.reference.value):
            next_sent_index = all_sents.index(curr_node.reference.value)-index_dif #all_sents lacks eof
            ...
            next_sents.append(next_vector)
            curr_targets.append(curr_target)
        curr_node = curr_node.reference
    return next_sents,curr_targets
