...
def get_combined_sent_vectors(sents_ll,all_sents,sent_vectors,all_targets):
    ...
    while (curr_node != None and curr_node.reference !=None):
        ...
        if not is_eof(curr_node.reference.value):
            ...
            combined_vectors.append(np.concatenate((curr_vector,next_vector),axis=0))
            combined_targets.append(curr_target)
        curr_node = curr_node.reference
...
def main():
    ...
    combined_sent_vectors,combined_targets = get_combined_sent_vectors(sents_ll,all_sents,sent_vectors,all_targets)
    
    test_model_cyber(combined_sent_vectors,combined_targets)
