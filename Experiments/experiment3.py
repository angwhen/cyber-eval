...
def modify_nexts_of_positives(x_train,vectors_to_modify,replacement):
    for i in range(0,len(x_train)):
        curr_vec = x_train[i]
        if (vectors_to_modify == curr_vec).all(1).any():
            x_train[i] = (x_train[i]+replacement)/2
    return x_train
...
def test_model_cyber(vectors,all_targets, vectors_to_modify, replacement):
    ...
    x_train = modify_nexts_of_positives(x_train,vectors_to_modify,replacement)
