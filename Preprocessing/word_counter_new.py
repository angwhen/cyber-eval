    ...
    for sent,pos_sent in zip(sents,pos_simple):
        ...
        for word,word_pos in zip(sent,pos_sent):
            try:
                ...
                new_sent.append(word) # if cant lemmatize, ie like EOF
...
def main():
    ...
    all_sents = pickle.load(open("all_sents.p", "rb"))
    all_pos = pickle.load(open("all_pos.p", "rb"))
   
    #processed_sents = lemmatize_sents(all_sents,get_simple_pos(all_pos))
    processed_sents = group_num(all_sents ,all_pos)
    ...
    save_processed(processed_sents,"grouped_all_sents_no_lemma_lcase.p")
